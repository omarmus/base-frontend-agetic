<template>
  <section>
    <h3 class="primary--text"><v-icon color="primary">business</v-icon> {{ !$util.isRol('SUPERADMIN') ? 'Entidad' : $t('entity.title') }}</h3>
    <v-card>
      <v-card-text>
        <crud-table
          :headers="headers"
...
export default {
  ...
  mounted () {
    if (this.$util.isRol('ADMIN', 'USUARIO')) {
      this.editItem(this.user.id_entidad, 'entidad', this.dataGraphql, false);
    }
  },
  data () {
    return {
      ...
      if (this.$refs.form.validate()) {
        ...
        if (data.id) {
          ...
          this.$service.graphql({
            query: `
              mutation edit($id: ID!, $entidad: EditEntidad!) {
